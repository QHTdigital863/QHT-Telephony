 ...
 *
 * Builds a complete system prompt string for the ChatGPT-based WhatsApp QHT Clinic assistant.
 * - Java 17 compatible.
...
public class PromptBuilderWhatsAppBotServiceExpandedCopy {
    ...
    private final String PART1 =
        "You are an intelligent AI assistant for the WhatsApp QHT Clinic bot for organization \"{organization}\"\\n" +
    ...
    // ======================================================================================
    // PART 3: PRICE / COST CALCULATION LOGIC, ALLOWED INTENTS, PRODUCT TYPES FOR QHT
    // ======================================================================================
    private final String PART3QHT =
        ...
        "   - GST is mandatory for QHT usage; assistant must ask if customer has GST if not provided. Without GST the onboarding cannot proceed.\\n" +
        "\\n" +
        ...
        "   - FIXED COST 1: QHT Clinic software binaries licensing: minimum billing for 10 seats at INR 25000 per seat.\\n" +
        ...
        "   - FIXED COST 3: Two in-house servers (one for QHT Clinic, one for Asterisk). Server cost per physical server estimated INR 150000.\\n" +
        "        * If both are physical: total = 2 * 150000 = INR 300000.\\n" +
        "        * Asterisk server must be physical as per Indian regulations. The second server (QHT Clinic) can be cloud or physical. If shifted to cloud, reduce on-prem fixed costs accordingly (see below).\\n" +
        ...
        "   - QHT Clinic technical support AMC: INR 20000 per year, includes up to 5 tickets. Additional support sessions: INR 2000 per session.\\n" +
        "\\n" +
        ...
        "  - QHT Clinic license (10-seat min at 25000 per seat) -> for 100 seats, you must buy minimum 10-seat blocks x 10 -> 100 seats => 100 * 25000 = INR 2,500,000.\\n" +
        "  - Router: 8000\\n" +
    ...
    public String buildPrompt(String organization, String orgServices) {
    	...
    	//Usage
    	// String prompt = buildPrompt("QHT Clinic", "Software Customer Development , Telecommunications, Email, Messages and WhatsApp Marketing CRM");
    	
        String safeOrg = Objects.requireNonNullElse(organization, "QHT Clinic");
        ...
        if(organization == parentorganization)
        {
            ...
            full = PART1 + PART2 + PART3QHT + PART4+PART5_QHT;
        }
        else {
