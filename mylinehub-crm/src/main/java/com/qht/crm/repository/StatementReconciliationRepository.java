...
public interface StatementReconciliationRepository extends JpaRepository<StatementReconciliation, Long> {
	...
	@Query("select  new com.qht.crm.entity.StatementReconciliation(e.id,e.name, e.organization, e.byExtension, e.client , e.reportHeading, e.bankName, e.noObservationFound , e.numberOfParallelThreads, e.createdOn) from StatementReconciliation e where e.organization = ?1")
