...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    @Query("select  new com.qht.crm.entity.Product(e.id,e.name, e.productType, e.productStringType, e.units , e.sellingPrice, e.purchasePrice, e.taxRate , e.unitsOfMeasure, e.organization,e.imageName,e.imageType) from Product e where e.organization = ?1")
    List<Product> findAllByOrganization(String organization);
    
    @Query("select  new com.qht.crm.entity.Product(e.id,e.name, e.productType, e.productStringType, e.units , e.sellingPrice, e.purchasePrice, e.taxRate , e.unitsOfMeasure, e.organization,e.imageName,e.imageType) from Product e where e.productStringType = ?1 and e.organization = ?2")
