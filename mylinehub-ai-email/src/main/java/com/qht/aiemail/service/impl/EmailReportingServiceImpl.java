...
import com.qht.aiemail.service.EmailReportingService;
...
public class EmailReportingServiceImpl implements EmailReportingService {
    ...
    public void reportEmail(EmailReportDTO dto) {
        ...
        String baseUrl = configService.getOptional("QHT_BASE_URL", null);
        String path = configService.getOptional("EMAIL_REPORT_URL", null);

        if (baseUrl == null || baseUrl.trim().isEmpty()) {
            log.warn("[EmailReport] QHT_BASE_URL not configured. Skipping HTTP call.");
            return;
        }
